...
- 外部智能体可以两种方式接入：
  - **官方 MCP 直连**：`http://你的MP地址:3000/api/v1/mcp`，认证头 `X-API-KEY=你的 MoviePilot API_TOKEN`
...
- 如果你主要想调用 `MoviePilot` 原生工具，可以先走官方 MCP
- 如果你要稳定跑资源流、固定命令、编号选择和修复链，第一步还是安装 `agent-resource-officer` 的 `skill / helper`
...
- `影巢搜索 <片名>`：只看影巢
- `云盘搜索 <片名>`：盘搜 + 影巢
- `MP搜索 <片名>` / `PT搜索 <片名>`：走 MoviePilot 原生搜索/PT；片名有歧义时先选正确的 MP/TMDB 候选，再进入 PT 资源
...
- `转存 <片名>`：默认等同 `115转存 <片名>`，只从 115 资源里择优转存
