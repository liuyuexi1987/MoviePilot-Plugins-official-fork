...
GLOBAL_CONFIG_PATH="${HOME}/.config/agent-resource-officer/config"
DRY_RUN=0
...
if [[ -f "${TARGET_DIR}/config" ]]; then
  ...
  cp "${TARGET_DIR}/config" "${CONFIG_BACKUP_DIR}/config"
fi
...
if [[ -n "${CONFIG_BACKUP_DIR}" && -f "${CONFIG_BACKUP_DIR}/config" ]]; then
  cp "${CONFIG_BACKUP_DIR}/config" "${TARGET_DIR}/config"
  rm -rf "${CONFIG_BACKUP_DIR}"
  echo "Preserved local skill config: ${TARGET_DIR}/config"
elif [[ -f "${GLOBAL_CONFIG_PATH}" ]]; then
  cp "${GLOBAL_CONFIG_PATH}" "${TARGET_DIR}/config"
  echo "Seeded local skill config from global config: ${TARGET_DIR}/config"
else
  echo "No skill-local config found. Create one when you need per-client isolation: ${TARGET_DIR}/config"
fi
