...
- Finalized the reduced public command surface for external agents: title-level cloud transfer/search composites stay deprecated, status aliases are redirected to the standard commands, and the helper-facing docs now consistently point agents to explicit prefixed search and numbered follow-up flows.

## 0.1.50

- helper 现在优先读取已安装 skill 目录里的本地 `config`，没有本地配置时才回退到 `~/.config/agent-resource-officer/config`
- `install.sh` 首次安装会把当前全局配置复制到 `<target>/config`，后续重装会保留本地 config，避免 Hermes / WorkBuddy / OpenClaw 互相串配置
- `config-check` / `readiness` 现在会明确显示当前实际使用的是本地 config 还是全局 config
